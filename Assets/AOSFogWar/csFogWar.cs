...
namespace FischlWorks_FogWar
{
    ...
    public class csFogWar : MonoBehaviour
    {
        ...
        private void InitializeFog()
        {
            ...
            MeshRenderer fogRenderer = fogPlane.GetComponent<MeshRenderer>();
                if (fogRenderer != null)
                {
                 // 2D-renderöintiin tarvitaan korkea SortingOrder.
                // Valitse luku, joka on korkeampi kuin minkään muun piilotettavan objektin Order in Layer.
                fogRenderer.sortingOrder = 100; 
        
                 // Varmista vielä, että Sorting Layer on oikea (esim. "Fog" tai "Default")
                // Sen tulee olla renderöity viimeisenä (alin Layer) "Sorting Layers" listassa.
                // Voit myös asettaa Layerin koodissa, jos tarve vaatii:
                // fogRenderer.sortingLayerName = "Fog";
                }
        }
...
private void OnDrawGizmos()
{
    ...
    // Aseta Gizmojen rotaatio pystysuoraan (X=90).
    // Tämä kääntää koko piirtokoordinaatiston, jolloin Z-akseli on "ylös" ja Y-akseli on "syvyys" (kohti/poispäin kamerasta).
    // Koska FOW Manager piirtää XZ-tasoon, tämä saa Gizmot vastaamaan FOG Planea.
    Handles.matrix = Matrix4x4.TRS(Vector3.zero, Quaternion.Euler(90f, 0f, 0f), Vector3.one); 
    
    // Piirrä Gizmot uuteen, kierrettyyn koordinaatistoon

    for (int xIterator = 0; xIterator < levelDimensionX; xIterator++)
    {
        for (int yIterator = 0; yIterator < levelDimensionY; yIterator++)
        {
            // FOW Manager piirtää X, Y(korkeus), Z(syvyys).
            // Koska Z on nyt pystysuunta 2D-maailmassasi,
            // siirrä piirtopisteen Y-komponentti (korkeus) alas 2D-tasoosi (esim. 0f).

            Vector3 gizmoPosition = new Vector3(
                GetWorldX(xIterator),
                GetWorldY(yIterator), // ENNEN Z, NYT Y (Hahmon 2D-pystysuuntainen sijainti)
                0f);                  // ENNEN Y, NYT Z (Syvyys, asetettu nollaan)

            // Piirto-logiikka jatkuu käyttäen gizmoPosition-muuttujaa
            ...
            if (levelData[xIterator][yIterator] == LevelColumn.ETileState.Obstacle)
            {
                ...
                Handles.DrawWireCube(
                    gizmoPosition, // Käytä uutta sijaintia
                    new Vector3(
    ...
    // PALAUTA HANDLES.MATRIX JOTTA SE EI VAIKUTA MUIHIN GIZMOIHIN
